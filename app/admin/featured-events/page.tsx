...
export default function FeaturedEventsPage() {
  const { events, updateEvent, loading } = useAdminData();
  const [query, setQuery] = useState("");

  const featuredCount = useMemo(() => events.filter((e) => e.featured).length, [events]);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    // Featured first, then by id — makes the current selection easy to review.
    const sorted = [...events].sort((a, b) => Number(!!b.featured) - Number(!!a.featured) || a.id - b.id);
    if (!q) return sorted;
  ...
  const toggle = (id: number, current: boolean) => {
    ...
    if (ev) updateEvent({ ...ev, featured: !current });
  };
    ...
    <div style={{ padding: "32px 40px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 8 }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", marginTop: 6, maxWidth: 620 }}>
          Star the events you want highlighted in the <strong style={{ color: "rgba(255,255,255,0.65)" }}>Featured Events</strong> carousel
          on the homepage. If none are starred, the newest events are shown automatically.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 13 }}>No events found.</p>
      ) : (
          ...
          {filtered.map((ev) => {
            ...
            return (
              <button
                key={ev.id}
                type="button"
                onClick={() => toggle(ev.id, isFeatured)}
                style={{
              ...
              >
                {/* Poster (4:5) */}
                <div style={{ position: "relative", width: "100%", aspectRatio: "4 / 5", overflow: "hidden", background: "#000" }}>
                  {ev.image ? (
                    <img src={ev.image} alt={ev.title} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center", opacity: isFeatured ? 1 : 0.75 }} />
                  ) : (
                    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.2)", fontSize: 11 }}>No poster</div>
                  )}
                  {/* Star badge */}
                  <div style={{
